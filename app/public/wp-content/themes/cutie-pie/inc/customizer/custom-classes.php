...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    class Cutie_Pie_Custom_Radio_Image_Control extends WP_Customize_Control {

        public $type = 'cutie-pie-radio-image';
...
// Radio COlor Schema
class Cutie_Pie_Custom_Radio_Color_Schema extends WP_Customize_Control {
 ...
 */
class Cutie_Pie_Dropdown_Taxonomies_Control extends WP_Customize_Control {
 ...
 * @access public
 */
class Cutie_Pie_Customize_Section_Upsell extends WP_Customize_Section {
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    // Repeator Info
    class Cutie_Pie_Premium_Notiece_Control extends WP_Customize_Control {

        public $type = 'cutie-pie-premiuminfonotice';
            ...
            <span class="customize-control-title">
                <div class="theme-info-icon">
                    <div class="icon-notice-wrap">
                        ...
                        <span><?php echo esc_html__('More ','cutie-pie'). esc_html( $this->label ). esc_html__(' Available on Premium Version.', 'cutie-pie' ); ?></span>
                    </div>
                </div>
            </span>
...
*/
class Cutie_Pie_Repeater_Controler extends WP_Customize_Control {
    /**
     * The control type.
     *
     * @access public
    ...
    */
    ...
    public $cutie_pie_box_label = '';

    public $cutie_pie_box_add_control = '';

    public $cutie_pie_box_add_button = '';
    ...
    public function __construct( $manager, $id, $args = array(), $fields = array() ) {
        $this->fields = $fields;
        $this->cutie_pie_box_label = $args['cutie_pie_box_label'] ;
        $this->cutie_pie_box_add_control = $args['cutie_pie_box_add_control'];
        $this->cutie_pie_box_add_button = $args['cutie_pie_box_add_button'];
    ...
    public function render_content() {
        ...
        <ul class="cutie-pie-repeater-field-control-wrap">
            <?php
            $this->cutie_pie_get_fields();
            ?>
        </ul>
        <input type="hidden" <?php esc_attr( $this->link() ); ?> class="cutie-pie-repeater-collector" value="<?php echo esc_attr( $this->value() ); ?>" />
        <?php

        if( isset( $this->cutie_pie_box_add_button ) && $this->cutie_pie_box_add_button ){ ?>
            
            <button type="button" class="button cutie-pie-add-control-field"><?php echo esc_html( $this->cutie_pie_box_add_control ); ?></button>
        <?php
        }
    ...
    private function cutie_pie_get_fields(){
        ...
        ?>
        <li class="cutie-pie-repeater-field-control">

        <div class="title-rep-wrap">
            <span class="dashicons dashicons-move twp-filter-icon"></span>
            <h3 class="cutie-pie-repeater-field-title"><?php echo esc_html( $this->cutie_pie_box_label ); ?></h3>
            <span class="dashicons dashicons-arrow-down twp-filter-icon"></span>
        </div>

        <div class="cutie-pie-repeater-fields">
        <?php
            foreach ($fields as $key => $field) {
            $class = isset($field['class']) ? $field['class'] : '';
            ?>
            <div class="cutie-pie-fields cutie-pie-type-<?php echo esc_attr($field['type']).' '.esc_attr( $class ); ?>">
                <?php 
                    ...
                    switch ($field['type']) {
                        ...
                        case 'selector_color':
                        $options = $field['options'];
                            echo '<div class="cutie-pie-type-radio">';
                        ...
                        case 'upload':
                                ...
                                esc_html_e('No image selected', 'cutie-pie');
                                echo '</div>';
                                echo '<div class="thumbnail thumbnail-image">';
                                echo $image;
                                echo '</div>';
                                echo '<div class="actions clearfix">';
                                echo '<button type="button" class="button theme-image-delete align-left">'.esc_html__('Remove', 'cutie-pie').'</button>';
                                echo '<button type="button" class="button theme-custom-upload-button alignright">'.esc_html__('Select Image', 'cutie-pie').'</button>';
            ...
            <div class="clearfix cutie-pie-repeater-footer">
                <div class="alignright">
                <?php if( isset( $this->cutie_pie_box_add_button ) && $this->cutie_pie_box_add_button ){ ?>
                    <a class="cutie-pie-repeater-field-remove" href="#remove"><?php esc_html_e('Delete', 'cutie-pie') ?>|</a> 
                <?php } ?>
                <a class="cutie-pie-repeater-field-close" href="#close"><?php esc_html_e('Close', 'cutie-pie') ?></a>
                </div>
            </div>
        </div>
        </li>
